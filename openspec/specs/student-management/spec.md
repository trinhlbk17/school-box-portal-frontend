...
## Purpose
Defines the student management feature for the Admin portal. Covers the full CRUD lifecycle for students within a class, the standalone StudentDetailPage, and integration with the ClassDetailPage Students tab.
...
- **THEN** the system SHALL open the StudentFormSheet in create mode
...
The system SHALL provide a `StudentFormSheet` component as a Sheet (side panel) for creating and editing students.

#### Scenario: Create mode
- **WHEN** StudentFormSheet opens without a student prop
- **THEN** it SHALL display empty fields and submit via `useCreateStudent`

#### Scenario: Edit mode
- **WHEN** StudentFormSheet opens with a student prop
...
- **THEN** the system SHALL open the StudentFormSheet in edit mode
...
- **THEN** the system SHALL render tabs for Info and Protectors
